...
public class SettingsManager : MonoBehaviour
{
    ...
    private int pendingDifficultyLevel = 1;
    void Awake()
    {
        // Singleton setup
        if (Instance == null)
    ...
    void Update()
    {
        // Ensure settings overlay remains interactable when it's open
        if (isSettingsOpen && overlayCanvasGroup != null && !overlayCanvasGroup.interactable)
    {
        Debug.LogWarning("Settings overlay became non-interactable - fixing it");
        EnsureSettingsInteractable();
    }
    }
    void InitializeUI()
    {
    ...
    System.Collections.IEnumerator FadeOut()
    {
        ...
        // Apply pending changes AFTER the menu is closed and game is resumed
        ApplyPendingChanges();
    }
    ...
    void OnDifficultyChanged(int value)
    {
        ...
        // Ensure the settings remain interactable after the change
        EnsureSettingsInteractable();
    }
    void EnsureSettingsInteractable()
    {
        if (isSettingsOpen && overlayCanvasGroup != null)
        {
            overlayCanvasGroup.interactable = true;
            overlayCanvasGroup.blocksRaycasts = true;
            overlayCanvasGroup.alpha = 1f;
            
            Debug.Log("Forced settings overlay to remain interactable");
        }
    }
    ...
    void ApplyPendingChanges()
    {
        ...
        {
            // Apply difficulty changes to the tile manager
            DynamicTileManager tileManager = FindFirstObjectByType<DynamicTileManager>();
            if (tileManager != null)
            {
                int minNum, maxNum;
                switch (pendingDifficultyLevel)
                {
                    case 0: // Low
                        minNum = 1;
                        maxNum = 5;
                ...
                Debug.Log($"Applying difficulty change: {GetDifficultyName(pendingDifficultyLevel)} (min: {minNum}, max: {maxNum})");
                tileManager.SetDifficulty(minNum, maxNum);
            }
